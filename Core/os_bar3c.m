% bar3c edit 
% 1. to handle NAN
% 2. to specify color bar limits (Clim)
% version 8 September 2016


function H = os_bar3c(Z,x,y,w,C,Clim)
...
if nargin < 6 || isempty(Clim); Clim = []; end
...
%% bar coloring
if ismatrix(C) && isempty(Clim)
    r = ceil((Z-min(Z(:)))/(max(Z(:))-min(Z(:)))*size(C,1));
    r(r(:)==0) = 1;
end
if ismatrix(C) && ~isempty(Clim)
    r = ceil((Z-Clim(1))/(Clim(2)-Clim(1))*size(C,1));
    r(r(:)==0) = 1;
end
